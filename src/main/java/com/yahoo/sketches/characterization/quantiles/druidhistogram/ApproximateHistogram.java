...
public class ApproximateHistogram
{
  ...
  public ApproximateHistogram copy(ApproximateHistogram h)
  {
    ...
    System.arraycopy(h.positions, 0, positions, 0, h.binCount);
    System.arraycopy(h.bins, 0, bins, 0, h.binCount);
    min = h.min;
    max = h.max;
    binCount = h.binCount;
    count = h.count;
  ...
  {
    // find common min / max
    float mergedMin = min < h.min ? min : h.min;
    float mergedMax = max > h.max ? max : h.max;
    long mergedCount = count + h.count;

    int maxSize = binCount + h.binCount;
    ...
    int numMerge = mergedBinCount - size;
    if (numMerge < 0) {
      numMerge = 0;
    }
    ...
    while (i < mergedBinCount) {
      positions[k] = mergedPositions[i];
      bins[k] = mergedBins[i];
      ++k;
      i = next[i];
    }
    binCount = mergedBinCount - numMerge;
    min = mergedMin;
    max = mergedMax;
    count = mergedCount;
    return this;
  ...
  protected ApproximateHistogram foldRule(ApproximateHistogram h, float[] mergedPositions, long[] mergedBins)
  {
    ...
    if (mergedPositions == null) {
      mergedPositions = new float[size];
      mergedBins = new long[size];
    }

    int mergedBinCount;
    if ((binCount + h.binCount) <= size) {
    ...
    for (int i = 0; i < mergedBinCount; ++i) {
      positions[i] = mergedPositions[i];
      bins[i] = mergedBins[i];
    }

    binCount = mergedBinCount;
    count = mergedCount;
  ...
  )
  {
    ...
    if ((upperLimit != Float.POSITIVE_INFINITY) && (lowerLimit != Float.NEGATIVE_INFINITY)) {
      cutoff = (upperLimit - lowerLimit) / (size - 2 - 1);
    } else {
      if (upperLimit != Float.POSITIVE_INFINITY) {
        cutoff = (upperLimit - min) / (size - 2);
      } else if (lowerLimit != Float.NEGATIVE_INFINITY) {
        cutoff = (max - lowerLimit) / (size - 2);
      } else {
        cutoff = (max - min) / (size - 1);
      }
    }
  ...
  public float[] getQuantiles(float[] probabilities)
  {
    ...
    for (int j = 0; j < probabilities.length; ++j) {
      ...
      // ([pi+1, mi+1])
      final double s = probabilities[j] * count();
      ...
      if (i == 0) {
        ...
        final double uj = positions[i - 1] + ((positions[i] - positions[i - 1]) * z);
        ...
        // [min] (p0, 0) < uj < (p, m) or
        //                      (p, m) < uj < (pB+1, 0) [max]
        quantiles[j] = ((float) uj < max()) ? (float) uj : max();
      }
    }
