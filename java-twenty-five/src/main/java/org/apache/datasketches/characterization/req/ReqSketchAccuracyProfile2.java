...
//  private int[] advSeq3 = new int[3];
  ...
  private UpdatableQuantilesDoublesSketch[] errQSkArr;
...
//    for (int i = 0; i < sarr.length; i++) { advSeq3[i] = Integer.parseInt(sarr[i]); }
  ...
  private void configurePlotPoints() {
    ...
    errQSkArr = new UpdatableQuantilesDoublesSketch[numPlotPoints];
    final QuantilesDoublesSketchBuilder builder = QuantilesDoublesSketch.builder().setK(1 << errQSkLgK);
    for (int pp = 0; pp < numPlotPoints; pp++) {
  ...
  void doTrial() { //for all plot points
    ...
    UpdatableQuantilesDoublesSketch qSk = errQSkArr[pp];
