...
public class ConcurrentThetaMultithreadedSpeedProfile extends BaseUpdateSpeedProfile {
  ...
  public void configure() {
    ...
    if (offHeap) {
      ...
      wseg = null; //WritableMemory.allocate(maxSharedUpdateBytes);
    }
  ...
  protected class WriterThread extends ConcurrentTestThread {
    private UpdatableThetaSketch local;
    private final long start;
    private long i;
    private final int jump;

    //c-tor for thread-safe (concurrent) sketch
    public WriterThread(final UpdatableThetaSketchBuilder bldr, final long start) {
      this(start);
      local = bldr.buildLocal(sharedSketch);
    }

    //c-tor for lock-based sketch
    public WriterThread(final UpdatableThetaSketch sketch, final long start) {
      this(start);
      local = sketch;
    }
  ...
  protected class ReaderWriterThread extends WriterThread {
    ...
    public ReaderWriterThread(final UpdatableThetaSketchBuilder bldr, final long start, final double writesRatio) {
      super(bldr, start);
