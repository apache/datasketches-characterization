...
public class DoublesSketchAccuracyProfile extends BaseQuantilesAccuracyProfile {
  ...
  private QuantilesDoublesSketchBuilder builder;
  ...
  public void configure() {
    ...
    k = 1 << lgK;
    builder = QuantilesDoublesSketch.builder().setK(1 << lgK);
  ...
  public double getEpsilon() {
    return QuantilesDoublesSketch.getNormalizedRankError(k, false);
  }
  ...
  public double doTrial() {
    ...
    // build sketch
    final UpdatableQuantilesDoublesSketch updateSketch = builder.build();
    for (int i = 0; i < inputValues.length; i++) {
      updateSketch.update(inputValues[i]);
    }

    final QuantilesDoublesSketch sketch = useCompact ? updateSketch.compact() : updateSketch;
