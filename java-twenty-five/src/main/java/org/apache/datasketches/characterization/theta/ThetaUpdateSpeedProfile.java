...
public class ThetaUpdateSpeedProfile extends BaseUpdateSpeedProfile {
  protected UpdatableThetaSketch sketch;
  ...
  public void configure() {
    ...
    final int k = 1 << lgK;
    final UpdatableThetaSketchBuilder udBldr = UpdatableThetaSketch.builder()
        .setNominalEntries(k)
        .setFamily(family)
        .setP(p)
        .setResizeFactor(rf);
    if (offheap) {
      final int bytes = UpdatableThetaSketch.getMaxUpdateSketchBytes(k);
      wseg = Arena.ofConfined().allocate(bytes);
      sketch = udBldr.build(wseg);
    } else {
      sketch = udBldr.build();
    }
  }
