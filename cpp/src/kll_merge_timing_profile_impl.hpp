...
#ifndef KLL_MERGE_TIMING_PROFILE_IMPL_HPP_
#define KLL_MERGE_TIMING_PROFILE_IMPL_HPP_
...
template<typename T>
kll_merge_timing_profile<T>::kll_merge_timing_profile():
generator(std::chrono::system_clock::now().time_since_epoch().count()),
distribution(0.0, 1.0)
{}

template<typename T>
void kll_merge_timing_profile<T>::run() {
  ...
  std::unique_ptr<kll_sketch<T>> sketches[num_sketches];
  ...
  while (stream_length <= (1 << lg_max_stream_len)) {
    ...
    for (size_t t = 0; t < num_trials; t++) {
      ...
      for (size_t i = 0; i < num_sketches; i++) {
        sketches[i] = std::unique_ptr<kll_sketch<T>>(new kll_sketch<T>());
      }
      kll_sketch<T> merge_sketch;
...
template<>
float kll_merge_timing_profile<float>::sample() {
  return distribution(generator);
}

template<>
std::string kll_merge_timing_profile<std::string>::sample() {
