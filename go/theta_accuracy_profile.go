...
type ThetaAccuracyProfile struct {
...
func MustNewThetaAccuracyProfile(cfg thetaJobConfig) *ThetaAccuracyProfile {
...
func (p *ThetaAccuracyProfile) run() {
...
func (p *ThetaAccuracyProfile) runTrial(key uint64) uint64 {
	p.sketch.Reset()
	...
	for _, stat := range p.stats {
		...
		est := p.sketch.Estimate()
		lb3, _ := p.sketch.LowerBound(3)
		lb2, _ := p.sketch.LowerBound(2)
		lb1, _ := p.sketch.LowerBound(1)
		ub1, _ := p.sketch.UpperBound(1)
		ub2, _ := p.sketch.UpperBound(2)
		ub3, _ := p.sketch.UpperBound(3)
...
func (p *ThetaAccuracyProfile) setHeader(sb *strings.Builder) string {
...
func (p *ThetaAccuracyProfile) process(cumTrials int, sb *strings.Builder) {
	points := len(p.stats)
