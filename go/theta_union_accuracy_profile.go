...
type ThetaUnionAccuracyProfile struct {
...
func MustNewThetaUnionAccuracyProfile(cfg thetaJobConfig) *ThetaUnionAccuracyProfile {
	union, err := theta.NewUnion(
		theta.WithUnionLgK(cfg.lgK),
		theta.WithUnionSketchP(cfg.p),
		theta.WithUnionResizeFactor(cfg.resizeFactor),
	)
	if err != nil {
		panic(err)
	}
	...
	baseStats := buildLog2AccuracyStatsArray(cfg.lgMinU, cfg.lgMaxU, cfg.uppo, cfg.lgQK)
	for _, stat := range baseStats {
		s := stat.(*accuracyStats)
		stats = append(stats, s)
	}

	return &ThetaUnionAccuracyProfile{
		config:    cfg,
		union:     union,
		sketch:    sketch,
		stats:     stats,
...
func (p *ThetaUnionAccuracyProfile) run() {
...
func (p *ThetaUnionAccuracyProfile) runTrial(key uint64) uint64 {
	p.union.Reset()
	...
	for _, stat := range p.stats {
		...
		for u := uint64(0); u < delta; u++ {
			key++
			p.sketch.UpdateUint64(key)
		}
		lastUniques += delta

		p.union.Update(p.sketch)

		result, _ := p.union.OrderedResult()
...
func (p *ThetaUnionAccuracyProfile) setHeader(sb *strings.Builder) string {
...
func (p *ThetaUnionAccuracyProfile) process(cumTrials int, sb *strings.Builder) {
	points := len(p.stats)
